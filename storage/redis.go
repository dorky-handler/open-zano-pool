...
func (r *RedisClient) WriteShare(login, id string, params []string, diff int64, height uint64, window time.Duration ,ip string) (bool, error) {
...
func (r *RedisClient) WriteBlock(login, id string, params []string, diff, roundDiff int64, height uint64, window time.Duration , ip string) (bool, error) {
	exist, err := r.checkPoWExist(height, params)
	if err != nil {
		return false, err
	}
	...
	if err != nil {
		...
		log.Printf("string is "+cmds[11].String())
		return false, cmd.Err()
		//return false, true
	}
}

func (r *RedisClient) writeShare(tx *redis.Multi, ms, ts int64, login, id string, diff int64, expire time.Duration , ip string) {
	tx.HIncrBy(r.formatKey("shares", "roundCurrent"), login, diff)
	tx.ZAdd(r.formatKey("hashrate"), redis.Z{Score: float64(ts), Member: join(diff, login, id, ms, ip)})
...
func (r *RedisClient) GetThreshold(login string) (int64, error) {
	...
	return cmd.Int64(),cmd.Err()
}

func (r *RedisClient) SetThreshold(login string, threshold int64) (bool, error) {
	cmd, err := r._leadClient.HSet(r.formatKey("settings", login), "payoutthreshold", strconv.FormatInt(threshold, 10)).Result()
	return cmd, err
}
...
func (r *RedisClient) CollectStats(nanoWindow time.Duration,smallWindow time.Duration, maxBlocks, maxPayments int64) (map[string]interface{}, error) {
	...
	totalHashrate, miners := convertMinersStats(nwindow,window, cmds[1].(*redis.ZSliceCmd))
	stats["miners"] = miners
...
func (r *RedisClient) CollectWorkersStats(nWindow,sWindow, lWindow time.Duration, login string) (map[string]interface{}, error) {
	nanoWindow := int64(nWindow / time.Second)
...
func convertMinersStats(nwindow int64,window int64, raw *redis.ZSliceCmd) (int64, map[string]Miner) {
	now := util.MakeTimestamp() / 1000
